...
def print_config_summary():
    ...
    if CAPTION_MODE == "filename":
        print(f"  Filename-map tab: {get_filename_map_tab_name()}")
...
_filename_map_cache = None  # populated once per process run, then reused

# Caption mode: "random" = pick any row from the caption tab (existing behaviour)
#               "filename" = match the media filename to a row in the filename-map tab
CAPTION_MODE = (get_env("CAPTION_MODE", required=False) or "random").strip().lower()
...
def load_filename_map():
    ...
    values = result.get("values", [])
    if len(values) < 2:
        print(f"Warning: filename-map tab '{tab_name}' has no data rows yet.")
        _filename_map_cache = {}
        return _filename_map_cache
...
def build_post(tags: list[str], with_link: bool, media_name: str = "") -> TextBuilder:
    tb = TextBuilder()
...
def post_video_to_bluesky(client, video_name, local_path, tags, with_link):
    text_builder = build_post(tags, with_link, media_name=video_name)
...
def post_image_to_bluesky(client, image_name, local_path, tags, with_link):
    text_builder = build_post(tags, with_link, media_name=image_name)
    with open(local_path, "rb") as f:
        image_bytes = f.read()
    client.send_image(
        text=text_builder,
